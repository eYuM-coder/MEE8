...
const PogyClient = require('./MEE8.js')
  ...
  async function checkExpiredWarnings() {
    ...
    try {
      ...
      if (error.name === 'MongoServerError' && error.code === 50) {
        logger.error('Query timed out after 5000ms. Consider optimizing the database.', {
          label: 'ERROR',
        })
      } else {
  ...
  try {
    try {
      await logger.info(
        ...
        }) ran by "${interaction.user.username}" (${interaction.user.id}) on guild "${
          interaction.guild.name
        }" (${interaction.guild.id}) in channel "${interaction.channel.name}" (${
      ...
      await logger.info(
        `"${interaction}" (${slashCommand.data.name}${
          subcommand ? ` ${subcommand}` : ''
        }) ran by "${interaction.user.username}" (${interaction.user.id}) in a DM.`,
        { label: 'Slash Commmand' },
      )
    }
...
const levelupbutton = new MessageEmbed()
  ...
  .setTitle('Level Up')
  .setFooter(`${config.botName}`, `${process.env.AUTH_DOMAIN}/assets/images/pogy.png`)
  .setDescription(`Hmmm... This doesnt seem to do much, but you can click it anyways.`)
  .setURL(`${process.env.AUTH_DOMAIN}/invite`)
  ...
  try {
    ...
    if (interaction.customId === 'support') {
      ...
      const resultEmbed = new MessageEmbed()
        ...
        .setDescription(`You chose ${emojis[userChoice]}, and the bot chose ${emojis[botChoice]}.`)
...
async function startTetrisGame(message) {
  ...
  function rotateTetromino(tetromino, direction) {
    const rotationMatrix =
      direction === 'clockwise' ? CLOCKWISE_ROTATION_MATRIX : COUNTERCLOCKWISE_ROTATION_MATRIX
  ...
  function canMove(gameState, direction) {
    ...
    const newCol =
      gameState.tetrominoCol + (direction === 'left' ? -1 : direction === 'right' ? 1 : 0)
