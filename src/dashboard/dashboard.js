...
module.exports = async client => {
  ...
  app.get(
    ...
    function (req, res, next) {
      return passport.authenticate('discord', { failureRedirect: '/' }, async function (err, user) {
        if (err) {
          console.error(err)
          return res.redirect('/')
        }
  ...
  async function getDashboardData(req, guildId) {
    ...
    const member = await guild.members.fetch(req.user.id)
    if (!member || !member.permissions.has('MANAGE_GUILD')) return { redirect: '/dashboard' }
  ...
  app.post('/dashboard/:guildID/embeds', checkAuth, async (req, res) => {
    ...
    if (req.body.type === 'embed') {
      ...
      if (!channel.permissionsFor(channel.guild.client.user).has('SEND_MESSAGES'))
...
In the mean time, please explain your issue below`
  ...
  app.post('/dashboard/:guildID/suggestions', checkAuth, async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'saveChannel')) {
      ...
      if (suggestionValid) {
        storedSettings.suggestion.suggestionChannelID = guild.channels.cache.find(
          ch => `#${ch.name}` === data.suggestionChannel,
        ).id
      } else {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'additional')) {
      ...
      if (suggestionValid) {
        storedSettings.suggestion.suggestionlogChannelID = guild.channels.cache.find(
